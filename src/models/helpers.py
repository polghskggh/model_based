...
def convolution_layer_init(features, kernel_size, strides, padding="VALID", std=np.sqrt(2), bias_const=0.0):
    layer = nn.Conv(features=features, kernel_size=(kernel_size, kernel_size), strides=(strides, strides),
                    padding=padding, kernel_init=nn.initializers.orthogonal(std),
                    bias_init=nn.initializers.constant(bias_const))
    return layer


def transpose_convolution_layer_init(features, kernel_size, strides, padding='VALID', std=np.sqrt(2), bias_const=0.0):
    layer = nn.ConvTranspose(features=features, kernel_size=(kernel_size, kernel_size), strides=(strides, strides),
                             padding=padding, kernel_init=nn.initializers.orthogonal(std),
                             bias_init=nn.initializers.constant(bias_const))
    return layer
