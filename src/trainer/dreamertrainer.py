...
class DreamerTrainer(Trainer):
    ...
    def loss_fun(models: dict, params: dict, data: tuple, rng: dict):
        ...
        for idx in range(len(states)):
            ...
            jax.debug.print("debug {done} {belief} {states}", done=jnp.astype(dones[idx], jnp.bool), belief=beliefs[idx].shape, states=states[idx].shape)
            belief = jnp.where(jnp.astype(dones[idx], jnp.bool), jnp.zeros_like(beliefs[idx], beliefs[idx]))
            state = jnp.where(jnp.astype(dones[idx], jnp.bool), jnp.zeros_like(states[idx]), states[idx])
